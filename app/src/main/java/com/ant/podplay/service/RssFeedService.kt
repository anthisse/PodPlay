...
import com.ant.podplay.BuildConfig
...
class RssFeedService private constructor() {
    ...
    suspend fun getFeed(xmlFileURL: String): RssFeedResponse? {
        ...
        // Create a FeedService
        val service: FeedService

        // Log events around this request
        val interceptor = HttpLoggingInterceptor()
        interceptor.level = HttpLoggingInterceptor.Level.BODY

        // Build the object with the RSS file URL
        ...
        // Make the feed URL end in a trailing "/" using split() with a regex
        val retrofit = Retrofit.Builder()
            .baseUrl("${xmlFileURL.split("?")[0]}/")
            .build()
        service = retrofit.create(FeedService::class.java)

        // Fetch the feed. If there's a result code >400, then the server had an error
        try {
            ...
            if (result.code() >= 400) {
                ...
                // Otherwise, return the success result and parse it
            ...
            // Catch any errors produced by the service
    ...
    private fun domToRssFeedResponse(node: Node, rssFeedResponse: RssFeedResponse) {
        ...
        // For each child node call build rssFeedResponse objects recursively
